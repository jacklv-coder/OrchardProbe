...
PR [#83](https://github.com/jacklv-coder/OrchardProbe/pull/83) is on
`main`; `LAB-002` is `done — retained No-Go`, and Issue #55 is closed.
Checkpoint 2 completed on `main` through merged PR #59. On 2026-07-31 the operator
...
activation [PR #90](https://github.com/jacklv-coder/OrchardProbe/pull/90)
authorizes checkpoint 2 device-free integration and synthetic tests after
merge. The [checkpoint-2 implementation ledger](docs/research/lab-004-device-free-integration.md)
records the opaque experiment-directory alignment, seven fixed Host profiles,
held preflight/closure boundary, external-input and diagnostic enforcement,
and synthetic regressions proposed by [PR #91](https://github.com/jacklv-coder/OrchardProbe/pull/91). It authorizes no
signing, upload, installation, envelope, device query or observation, backend
work, or IPA decryption.
...
| 5 | Sanitized LAB-002 Go/No-Go result | `done — PR #83` | [PR #83](https://github.com/jacklv-coder/OrchardProbe/pull/83) preserves the procedural 4C No-Go in the bilingual ledger and closes Issue #55 without private paths, identifiers, fingerprint, Receipt content, or Host results; no Go or backend claim follows |
...
| 15 | `LAB-004` | `active — checkpoint 2 done after PR #91 merges` | Evaluate one fresh first-party DemoLab `1.0 (4)` protected-oracle tuple using the completed LAB-003 role boundary. Checkpoint 2 aligns opaque Host publication and enforces seven fixed preflight/closure profiles with held external-input and diagnostic roles plus synthetic regressions. Checkpoint 3 remains an unauthorized proposal; signing, upload, installation, envelopes, device actions, backend claims, and IPA decryption remain separately gated. A documented No-Go is accepted. | `LAB-003` layout Go | [#89](https://github.com/jacklv-coder/OrchardProbe/issues/89) | [#90](https://github.com/jacklv-coder/OrchardProbe/pull/90) | Checkpoint 2: [#91](https://github.com/jacklv-coder/OrchardProbe/pull/91) |
...
`LAB-004` checkpoint 2 adds only device-free Host integration. None of these
