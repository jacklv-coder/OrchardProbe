...
| 14 | `LAB-003` | `active when this activation PR merges` | Make external Receipt/Export inputs and bounded diagnostics unambiguous before any future authorization exists. Publish the closed three-role layout and phase-aware experiment inventory, implement device-free path/identity validation and preflight UX, and retain all closed LAB-002 lifecycle guards. Completion may only permit a later documentation proposal; it does not authorize or perform a new device ceremony. | `LAB-002` retained No-Go | [#84](https://github.com/jacklv-coder/OrchardProbe/issues/84) | This activation PR | — |
