...
当前代码刻意只实现 Host 端基础能力：报告本机 pre-alpha 状态、读取单个本地 Mach-O 的有界 Header 元数据、执行仅限库内的有界 IPA Archive 预检与 Entry 流式读取、解析有界的根 App 与约定嵌套 Bundle 身份元数据及精确声明可执行 Entry、检查根主程序的 Mach-O 结构，并仅在 Mach-O 解析通过后构建确定性的声明标准 Bundle 清单、把已验证 IPA App 树物化到自动清理的私有有界工作树、把该未改变的工作树重新打包成确定性、未签名、仅供分析的 IPA，并把源/输出 Hash、清单、排除项、打包政策和每个确认 Mach-O Slice 绑定到诚实的无设备 Version-3 Manifest。当前没有 CLI 命令接受 IPA。
...
不会修改 Mach-O。[确定性 IPA Packager（英文）](docs/development/ipa-deterministic-package.md)
只能消费这个受控工作树，并返回私有、自动清理、字节级可复现且完成最终有界预检
的 IPA。它仍只是库内原语：不会发布调用方指定的输出、不会证明解密，也没有实现
`oprobe decrypt`。后续的[无设备打包证据 Builder（英文）](docs/development/ipa-package-manifest.md)
会重新验证并 Hash 两个完整 Archive 与每个确认 Code Entry，记录完整 Slice 和排除
证据，但仍只报告 `inconclusive` 结构证据。当前 CLI 不会发布产物或重新计算这些
证据。
