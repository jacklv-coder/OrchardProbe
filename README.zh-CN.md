...
当前代码刻意只实现 Host 端基础能力：报告本机 pre-alpha 状态、读取单个本地 Mach-O 的有界 Header 元数据、执行仅限库内的有界 IPA Archive 预检与 Entry 流式读取、解析有界的根 App 身份元数据及其声明主程序的 Mach-O 结构、输出确定性的合成 manifest，以及检查 manifest 的 Schema 与路径安全约束。当前没有 CLI 命令接受 IPA。
...
它不证明已安装构建匹配、解密或明文字节正确，也尚未枚举 Framework、dylib 或
Extension 代码。
