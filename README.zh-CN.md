...
当前代码刻意只实现 Host 端基础能力：报告本机 pre-alpha 状态、读取单个本地 Mach-O 的有界 Header 元数据、执行仅限库内的有界 IPA Archive 预检和单个有界内存 Entry 读取、输出确定性的合成 manifest，以及检查 manifest 的 Schema 与路径安全约束。当前没有 CLI 命令接受 IPA。
