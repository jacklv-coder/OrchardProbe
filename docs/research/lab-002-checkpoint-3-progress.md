...
| 3D.6 | Closed fixup identity across re-signing | `in implementation` | The one authorized run from merged `b601a4b...` used a fresh verified owner-only 3A tuple, completed signed Archive/export, and passed the closed extent plus normalized-prefix checks. It then safely rejected before oracle publication because `fixup_layout_sha256` also bound the permitted `__LINKEDIT.filesize` growth. Continue using the actual filesize for segment/fixup bounds, but normalize only the parsed `__LINKEDIT` filesize in the fixup identity; bind every other segment extent and complete fixup payload. Add positive and negative regressions, reproduce and admit the exact helper product, then pass Codex CR/CI/review and merge before another run |
