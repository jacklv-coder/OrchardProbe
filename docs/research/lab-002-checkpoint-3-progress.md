...
| 3D | Exact signed DemoLab `1.0 (3)` candidate | `active; closed export-signature-tail fix in implementation` | The first-party App IDs and dedicated App Group are privately configured and an independent temporary signing preflight passed. PRs #67-#69 closed the post-export XcodeGen check, export-only `Symbols/` sidecar, rollback classification, and reproducible private-helper allowlist. The next fresh source-bound run again completed signed Archive/export, then safely rejected the legitimate 64-byte App Store re-signing growth at the main-app and share-extension signature tails. No oracle/evidence or candidate was published; cleanup proof removed staging, and no upload, installation, or device observation occurred |
...
| 3D.5 | Closed App Store re-signing extent | `in implementation; reproducible helper reviewed` | The fresh `7c058004...` run proved that current Xcode keeps the same thin container, architecture, CPU subtype, UUID, fixed range, and signature start while growing only the trailing Code Signature/`__LINKEDIT` extent by 64 bytes for the main app and share extension. Accept only one-slice thin binaries whose Archive/IPA signature tails start together and each end exactly at its own EOF; retain all existing identity/range/fixup/encryption/signing checks and record the final IPA slice size. Two independent sandboxed builds of the exact fix commit produced the same private-helper size, full SHA-256, and CodeDirectory CDHash before its tuple was admitted |
