...
| 3D | Exact signed DemoLab `1.0 (3)` candidate | `active; Xcode export-policy fix in implementation` | The first-party App IDs and dedicated App Group are privately configured and an independent temporary signing preflight passed. [PR #67](https://github.com/jacklv-coder/OrchardProbe/pull/67) fixed the first post-export XcodeGen failure and merged as `c6e9bd8d6620564017bcce2e81a8dfc3fc41e72f`. A fresh source-bound run then reached signed Archive/export and the repaired check, but failed closed because current Xcode's default `uploadSymbols=YES` added a top-level `Symbols/` sidecar outside the deliberately closed IPA app root. No candidate was published and no upload, installation, or device observation occurred |
...
| 3D.4 | Reproducible private-helper product review | `in implementation` | The helper changed in PR #68, so the old product allowlist correctly blocked 3A preparation. Two independent builds from merged source `091be371...`, the pinned Rust 1.85.0 toolchain, verified offline dependencies, and the reviewed sandbox produced the same size, full SHA-256, and CodeDirectory CDHash; admit only that exact tuple through review and CI |
| 3D.5 | Fresh exact candidate | `blocked by 3D.4 merge` | From the clean merged allowlist commit, create a new source-bound 3A tuple and publish exactly one verified owner-only `1.0 (3)` Archive/IPA/oracle/evidence run; do not reuse either failed-run tuple and do not upload, install, or observe a device |

Neither failed run is a candidate: both Archive/export operations completed,
but neither published the closed oracle/evidence tuple. The first unpublished
...
non-candidate diagnostic name. Both source-bound pre-build tuples remain private
historical diagnostics and cannot be reused after a fix changes `main`.
