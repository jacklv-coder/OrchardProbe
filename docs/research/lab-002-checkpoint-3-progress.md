...
| 3B | Archive/oracle/evidence closure | `in progress; 3B.1 complete, 3B.2 final retry-gate fix CR/CI pending` | Make the hardened archive flow consume and revalidate the exact 3A artifacts, build only the three allowlisted roles, compare Archive and IPA slice identity plus `__TEXT,__oprobe`, publish the canonical frozen oracle, and bind its external SHA-256 into pre-upload evidence; the upload lane must reject absent or mismatched manifest/oracle evidence |
...
| 3B.2 | Archive/IPA oracle closure | `final retry-gate fix CR/CI/merge pending` | Measure the three allowlisted Archive and IPA executables, close slice/UUID/range/fixup identity, compare `__TEXT,__oprobe`, and atomically publish one canonical owner-only oracle |
...
| 3B.2.4 | Device-free closure tests | `local tests and helper reproduction complete; final retry-gate fix CR/CI pending` | Synthetic fixture tests cover parity success plus target, slice, UUID, range, fixup, plaintext, canonicalization, permission, substitution, and atomic-publication failures; documentation, Codex CR, and CI must pass before 3B.3 is activated |
