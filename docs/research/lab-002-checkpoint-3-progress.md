...
| 3D | Exact signed DemoLab `1.0 (3)` candidate | `active; PR #70 P1 fix in implementation` | The first-party App IDs and dedicated App Group are privately configured and an independent temporary signing preflight passed. PRs #67-#69 closed the post-export XcodeGen check, export-only `Symbols/` sidecar, rollback classification, and reproducible private-helper allowlist. The next fresh source-bound run again completed signed Archive/export, then safely rejected the legitimate 64-byte App Store re-signing growth at the main-app and share-extension signature tails. No oracle/evidence or candidate was published; cleanup proof removed staging, and no upload, installation, or device observation occurred. PR #70 is being hardened so the closed tail exception also binds the normalized complete pre-signature image |
...
| 3D.5 | Closed App Store re-signing extent | `in implementation; PR #70 P1 open` | The fresh `7c058004...` run proved that current Xcode keeps the same thin container, architecture, CPU subtype, UUID, fixed range, and signature start while growing only the trailing Code Signature/`__LINKEDIT` extent by 64 bytes for the main app and share extension. Accept only one-slice thin binaries whose Archive/IPA signature tails start together and each end exactly at its own EOF. Compare the complete pre-signature bytes after normalizing only the parsed `__LINKEDIT.filesize` and `LC_CODE_SIGNATURE.datasize` fields, retain all existing identity/range/fixup/encryption/signing checks, and record the final IPA slice size. Any adjacent or otherwise unlisted prefix-byte change fails closed. After this P1 fix is committed, two independent sandboxed builds of that exact commit must produce the same private-helper size, full SHA-256, and CodeDirectory CDHash before the tuple is admitted |
| 3D.6 | Fresh exact candidate | `blocked by 3D.5 merge` | From the clean merged signature-tail fix commit, create a new source-bound 3A tuple and publish exactly one verified owner-only `1.0 (3)` Archive/IPA/oracle/evidence run; do not reuse any failed-run tuple and do not upload, install, or observe a device |

None of the three signed runs is a candidate: every Archive/export operation
...
proved no oracle state and was durably cleaned. All source-bound pre-build tuples
remain private historical diagnostics and cannot be reused after a fix changes
`main`.
