...
| 3D.6 | Closed fixup identity across re-signing | `implementation complete; PR/CI pending` | The one authorized run from merged `b601a4b...` used a fresh verified owner-only 3A tuple, completed signed Archive/export, and passed the closed extent plus normalized-prefix checks. It then safely rejected before oracle publication because `fixup_layout_sha256` also bound the permitted `__LINKEDIT.filesize` growth. The implementation continues using the actual filesize for segment/fixup bounds, but normalizes only the parsed `__LINKEDIT` filesize in the fixup identity; every other segment extent and the complete fixup payload remain bound. Positive regressions permit only that resize, while ordinary-segment size and segment-label substitutions change the digest. Two independent sandboxed builds of exact fix commit `d737ce2...` produced identical private-helper size, full SHA-256, and CodeDirectory CDHash; the exact tuple was admitted and a third normal allowlist build passed. Codex CR/CI/review and merge remain required before another run |
