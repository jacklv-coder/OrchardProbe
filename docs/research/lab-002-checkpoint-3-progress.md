...
| 3D | Exact signed DemoLab `1.0 (3)` candidate | `active; PR #70 review and CI pending` | The first-party App IDs and dedicated App Group are privately configured and an independent temporary signing preflight passed. PRs #67-#69 closed the post-export XcodeGen check, export-only `Symbols/` sidecar, rollback classification, and reproducible private-helper allowlist. The next fresh source-bound run again completed signed Archive/export, then safely rejected the legitimate 64-byte App Store re-signing growth at the main-app and share-extension signature tails. No oracle/evidence or candidate was published; cleanup proof removed staging, and no upload, installation, or device observation occurred. PR #70 now binds the normalized complete pre-signature image and is awaiting final review/CI |
