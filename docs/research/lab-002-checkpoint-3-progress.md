...
| 3D.3 | Closed Xcode export policy and deterministic rollback | `in implementation` | Set `uploadSymbols=false` so the controlled IPA remains the single `Payload/*.app` tree while retaining the Archive dSYM separately; keep staging only for the helper's exact indeterminate-publication marker and clean deterministic pre-publication failures |
| 3D.4 | Fresh exact candidate | `blocked by 3D.3 merge` | From the clean merged fix commit, create a new source-bound 3A tuple and publish exactly one verified owner-only `1.0 (3)` Archive/IPA/oracle/evidence run; do not reuse either failed-run tuple and do not upload, install, or observe a device |
