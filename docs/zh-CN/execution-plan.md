...
Host 启动在发布前失败关闭，并暴露了被遮蔽的确认管道诊断；该修复须先通过评审并合并，
随后在安装 Enrollment 前紧邻地记录新的 RFC-0001 确认及签名一次性信封，并在每轮运行前另行记录
...
| 4 | 安装 Enrollment 与两次干净真机观察 | `active — 4C 诊断修复评审中` | 精确 Build 的有界流程已授权，恰好一次内部上传已完成远端对账，闭合 Host 操作流程已通过 PR #74 合并。第一次 4C Host 启动在发布前失败关闭，并暴露被遮蔽的确认管道诊断。按[检查点 4 进度台账](lab-002-checkpoint-4-progress.md)先合并修复，再在 Enrollment 与每次干净运行前紧邻地分别保留新的全新确认/信封 |
