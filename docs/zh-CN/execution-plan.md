...
初始保护/预期明文 Oracle 证据、脱敏、明确 Go/No-Go、文档和收窄声明标准。只有
这个只改文档的激活 PR 经过评审并合并后，研究实现才能开始；`DEVICE-001` 及以后
步骤保持未启动。
...
| 12 | `LAB-001` | `active` | 建立首方受保护 DemoLab Oracle，同时提供初始保护状态与预期明文的独立证据；否则记录有界 No-Go。 | `HOST-010` | [#9](https://github.com/jacklv-coder/OrchardProbe/issues/9) | 本激活 PR | — |
