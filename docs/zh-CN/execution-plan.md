...
`HOST-010` 是唯一正在激活的计划步骤。其带版本的源/产物 Hash、Canonical 清单
绑定、逐二进制未改变字节证据、排除项、诚实的 Inconclusive 语义、资源边界、
Schema 迁移、测试和文档验收标准由 Issue #43 固定。只有这个只改文档的激活 PR
经过评审并合并后，实现才能开始；`LAB-001` 及以后步骤不得启动。
...
| 11 | `HOST-010` | `active` | 使用无设备 Fixture，把输入/输出 Hash、清单、逐二进制状态、排除项和打包证据写入带版本 Manifest。 | `HOST-009` | [#43](https://github.com/jacklv-coder/OrchardProbe/issues/43) | [#44](https://github.com/jacklv-coder/OrchardProbe/pull/44) | — |
