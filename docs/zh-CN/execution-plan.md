...
合并；其[顺序进度台账](lab-002-checkpoint-4-progress.md)现在把 4C 列为下一步，要求在安装
Enrollment 前紧邻地记录新的 RFC-0001 确认及签名一次性信封，并在每轮运行前另行记录
...
| 4 | 安装 Enrollment 与两次干净真机观察 | `active — 4C 等待新确认` | 精确 Build 的有界流程已授权，恰好一次内部上传已完成远端对账，闭合 Host 操作流程已通过 PR #74 合并。按[检查点 4 进度台账](lab-002-checkpoint-4-progress.md)在 Enrollment 与每次干净运行前紧邻地分别保留全新确认/信封 |
