...
`HOST-009` 是唯一正在激活的计划步骤。其确定性 Archive 策略、安全元数据归一化、
保留工作树身份边界、输出上限、验证、清理行为和验收标准由 Issue #40 固定。
只有这个只改文档的激活 PR 经过评审并合并后，实现才能开始；`HOST-010` 及以后
步骤不得启动。
...
| 10 | `HOST-009` | `active` | 使用未改变的 Fixture 字节重建确定性、未签名、仅供分析的 IPA；保留必要元数据且绝不宣称已经解密。 | `HOST-008` | [#40](https://github.com/jacklv-coder/OrchardProbe/issues/40) | [#41](https://github.com/jacklv-coder/OrchardProbe/pull/41) | — |
...
`HOST-008` 之后的条目都是计划，不是已实现能力。仓库目前尤其没有设备后端、
