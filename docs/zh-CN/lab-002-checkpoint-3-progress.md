...
| 3D | 精确签名 DemoLab `1.0 (3)` 候选 | `进行中；PR #70 P1 修复实现中` | 首方 App ID 与专用 App Group 已在私有环境完成配置，独立临时签名预检通过。PR #67-#69 已闭合导出后 XcodeGen 复核、仅供导出的 `Symbols/` Sidecar、Rollback 分类及可复现私有 Helper Allowlist。下一次全新 Source-bound Run 再次完成签名 Archive/Export，随后安全拒绝主 App 与 Share Extension 在 App Store 重签名时各自合法增长 64 字节的签名尾。没有发布 Oracle/Evidence 或 Candidate；Cleanup Proof 已清理 Staging，也没有上传、安装或设备观察。PR #70 正在加固，使闭合签名尾例外同时绑定归一化后的完整签名前镜像 |
...
| 3D.5 | 闭合 App Store 重签名 Extent | `实现中；PR #70 P1 待闭合` | 全新 `7c058004...` Run 证明当前 Xcode 保持相同 Thin Container、架构、CPU Subtype、UUID、固定区间及签名起点，只为主 App 与 Share Extension 把末尾 Code Signature/`__LINKEDIT` Extent 增长 64 字节。仅接受单 Slice Thin Binary，要求 Archive/IPA 签名尾起点相同且各自精确结束于 EOF；只把解析出的 `__LINKEDIT.filesize` 与 `LC_CODE_SIGNATURE.datasize` 字段归零后比较全部签名前字节，并保留全部既有身份/区间/Fixup/Encryption/签名检查、记录最终 IPA Slice Size。相邻字节或其他任何未列出的前缀变化都闭合失败。P1 修复提交后，必须对该精确 Commit 独立沙箱构建两次，确认私有 Helper Size、完整 SHA-256 与 CodeDirectory CDHash 一致，才允许 Tuple 进入 Allowlist |
