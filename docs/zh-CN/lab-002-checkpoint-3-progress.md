...
| 3D | 精确签名 DemoLab `1.0 (3)` 候选 | `进行中；闭合 Fixup 身份后续修复实现中` | 首方 App ID 与专用 App Group 已在私有环境完成配置，独立临时签名预检通过。PR #67-#70 已闭合导出后 XcodeGen 复核、仅供导出的 `Symbols/` Sidecar、Rollback 分类、可复现私有 Helper Allowlist、签名尾 Extent 与归一化后的完整签名前镜像。PR #70 合并后的第一次全新 Run 再次完成签名 Archive/Export 并通过新前缀门禁，随后因既有 Fixup-layout 身份也 Hash 了允许增长 64 字节的 `__LINKEDIT.filesize` 而安全拒绝。没有发布 Oracle/Evidence 或 Candidate；Cleanup Proof 已清理 Staging，也没有上传、安装或设备观察 |
...
| 3D.6 | 闭合重签名后的 Fixup 身份 | `实现中` | 从已合并 `b601a4b...` 发起的唯一授权 Run 使用全新且经验证的 Owner-only 3A Tuple，完成签名 Archive/Export，并通过闭合 Extent 与归一化前缀检查。随后它在 Oracle 发布前安全拒绝，因为 `fixup_layout_sha256` 还绑定了获准变化的 `__LINKEDIT.filesize`。Segment/Fixup 边界仍使用实际 Filesize，但 Fixup 身份只归一化已解析的 `__LINKEDIT` Filesize；其他所有 Segment Extent 与完整 Fixup Payload 继续绑定。补齐正反回归、复现并登记精确 Helper 产品，再通过 Codex CR/CI/Review 与合并后才允许下一次运行 |
