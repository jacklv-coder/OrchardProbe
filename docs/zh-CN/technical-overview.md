...
| 有界协议规范 | 设计已接受；未实现 |
| 受保护首方 Oracle | LAB-001 得到有界 No-Go；LAB-002 已完成三个可执行文件完整安装 Slice 清单的无设备设计与实现，检查点 3 仅为 DemoLab `1.0 (3)` 候选/Oracle 激活，私有 3A 预构建生成器已在 [PR #62](https://github.com/jacklv-coder/OrchardProbe/pull/62) 实现并等待评审/合并；尚无签名候选、上传、真机验证或 Go 结果 |
| 设备发现与 Transport | 未实现 |
