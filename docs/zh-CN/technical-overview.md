...
> Mach-O 结构检查、版本化 Schema、合成 DemoLab Fixture 和有界协议规范，但还
> 没有设备 Transport、Helper、解密后端、重建器或 IPA Packager。
...
[IPA 主程序检查文档（英文）](../development/ipa-main-executable.md)。Framework、
dylib 和 Extension 候选发现仍未实现。
...
| IPA 根主程序 Mach-O 元数据 | 库内已实现；尚无 Framework/dylib/Extension 清单或 CLI 接入 |
...
5. 读 [IPA 主程序检查契约（英文）](../development/ipa-main-executable.md)，再跟踪
   ...
   清单漂移和 Mach-O 测试。
6. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
7. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
8. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v2.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
9. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
10. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
11. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
