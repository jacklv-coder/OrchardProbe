...
> Mach-O 结构检查，以及覆盖精确 Framework/Extension 声明与范围内小写 dylib 的
> 有界声明标准 Bundle 清单，以及自动清理、排除 Receipt/`SC_Info` 并阻止
> Symlink/Path Escape 的私有有界 IPA 工作树，以及从该工作树生成确定性、未签名、
> 仅供分析 IPA 的库内 Packager；Version-3 Manifest Builder 还会绑定完整
> Archive/清单 Hash、打包政策、排除项和每个确认 Mach-O Slice，同时保持
> Inconclusive 结果。不支持的覆盖范围保持显式。仓库另有合成 DemoLab Fixture 和
> 有界协议规范，但还没有设备 Transport、Helper、解密后端、Mach-O 重建器、产物
> 发布器或 `oprobe decrypt` 命令。
...
会在读取 Payload 前规划完整目标树，排除路径组件精确等于 `_MASReceipt` 或
`SC_Info` 的内容，并通过 Descriptor-relative、No-follow、Create-new 操作把其余
字节流式写入全新 `0700` RAII 工作树。文件固定为 `0600`，不复制 Archive 的
Ownership、时间、Attribute 或可执行位；Drop 或任何错误都会清理整棵树。该能力
仍只在库内，精确契约见[私有有界 IPA 工作树文档（英文）](../development/ipa-private-worktree.md)。

仅限 Unix 的
[`crates/orchardprobe-core/src/ipa_package.rs`](../../crates/orchardprobe-core/src/ipa_package.rs)
只能消费上述受控工作树，不能接受任意 Host 目录或调用方路径。它通过保留的
Descriptor 与身份重新打开封闭计划，按 Canonical 顺序把归一化 Entry 写入私有
`0600` 临时 Archive，执行 16 GiB 输出上限，并在返回只读访问前复用同一套有界
IPA 预检。字节与证据明确标记为 `unsigned_analysis_only`，不宣称已经解密。精确
契约见[确定性 IPA 打包文档（英文）](../development/ipa-deterministic-package.md)。

仅限 Unix 的
[`crates/orchardprobe-core/src/ipa_manifest.rs`](../../crates/orchardprobe-core/src/ipa_manifest.rs)
会重新验证并 SHA-256 Hash 两个完整 Archive，通过带版本 Canonical 摘要绑定其完整
清单，对每个确认 Code Entry 执行源/输出 Hash 与 Mach-O 重解析，并在 Manifest
Version 3 中记录打包政策、排除项、拒绝项和全部 Slice。相等的 Fixture Hash 仍只
是 `inconclusive` 结构证据。精确契约见
[无设备打包证据 Manifest 文档（英文）](../development/ipa-package-manifest.md)。
...
它只返回根元数据；独立的声明标准 Bundle 清单会消费这些结果，私有工作树层则
独立消费底层 Archive 清单。精确契约见
...
会为有界的约定 Framework 和直接 Extension plist 复用该 Event Parser，解析包括
非标准名称在内的精确声明可执行 Entry，但不会读取其 Payload 或把它称为 Mach-O。
精确契约见[嵌套 Bundle 元数据文档（英文）](../development/ipa-nested-bundles.md)。
...
会合并根 App、受支持嵌套 Bundle 的精确声明，以及同一封闭祖先范围内的小写
dylib。声明角色优先于文件后缀，不猜测 Bundle Stem；只有同一 Parser 通过后才
称为 Code，误报和畸形候选保持可见。Coverage 是
`declared_standard_bundles`，不代表任意 App Code 已完整覆盖。精确契约见
[声明 Code 清单文档（英文）](../development/ipa-code-inventory.md)。
...
进入主线。检查点 4 已以保留证据的流程性 No-Go 关闭：恰好一次内部 TestFlight 上传已
对账为 Apple 接受并处理完成，未启用外部测试或提交审核。
[检查点 4 台账](lab-002-checkpoint-4-progress.md)记录闭合 Host 操作流程及截至 PR #81
的此前修复。最后一次全新 4C 仪式通过 TestFlight 显示并打开首方 DemoLab `1.0 (3)`，
导入一次性信封，并创建、导出签名 Enrollment Receipt。随后 Host 因外部输入被放入严格
实验目录而在发布 Enrollment Binding 前拒绝其清单，所以已安装二进制 Lineage 仍未验证，
授权继而过期。两轮观察均未执行，不存在受保护到明文结果或 Go 结果。
...
当前库内 Packager 使用保留的 Directory Descriptor 与记录的身份遍历已验证工作
树。它只按封闭计划加入普通 Bundle 文件，固定 Path、时间、压缩、Comment 与 Mode
规则，拒绝变化或特殊 Node，也不复制 Ownership、特殊权限位、无关 Extended
Attribute、Receipt 或 App Data。返回值是自动清理的私有临时 IPA；最终有界预检
必须与预期 Path、Kind、Size 和 Executable Class 完全一致。

当前 Manifest Builder 已为无设备阶段绑定输入/输出 Hash、清单、打包政策、排除
项、声明 Code Coverage、拒绝项和逐二进制未改变字节。调用方指定目标、设备产生的
重建证据以及原子发布为 `*.decrypted.ipa` 仍属于后续步骤。把未改变的 Fixture
字节打包成功不能证明 Mach-O 已经重建或解密。
...
| `crates/orchardprobe-core/src/ipa_bundle.rs` | 有界约定嵌套 Framework/Extension 发现、plist 解析和精确声明可执行 Entry 定位。 |
| `crates/orchardprobe-core/src/ipa_code.rs` | 绑定完整清单的根主程序流式读取，以及通过匿名临时文件执行的有界 Mach-O 元数据检查。 |
| `crates/orchardprobe-core/src/ipa_catalog.rs` | 确定性的声明标准 Bundle 选择、有界 Mach-O 确认、优先级规则和可见拒绝原因。 |
| `crates/orchardprobe-core/src/ipa_materialize.rs` | 私有有界 IPA App 树规划、Descriptor-relative 物化、确定性排除记录和 RAII 清理。 |
| `crates/orchardprobe-core/src/ipa_package.rs` | 从保留的私有工作树确定性打包未签名分析 IPA，执行输出边界、最终预检和 RAII 清理。 |
| `crates/orchardprobe-core/src/ipa_manifest.rs` | 无设备 Archive/清单 SHA-256 绑定、逐 Code 结构/Hash 证据、完整 Slice、排除项和 Manifest-v3 构建。 |
...
未来 Transport、Backend、Mach-O Reconstruction、设备产生的证据与原子发布模块
...
| 有界根 Info.plist 身份解析 | XML/Binary Event 库内实现；尚未接入 CLI |
| 有界嵌套 Framework/Extension plist 元数据 | 库内已实现并由 Catalog 消费；尚未接入 CLI |
| IPA 根主程序 Mach-O 元数据 | 库内已实现；尚未接入 CLI |
| 声明标准 Bundle Code 清单 | 已覆盖根程序、受支持嵌套声明和范围内小写 dylib；任意 Bundle 覆盖与 CLI 接入仍不支持 |
| 私有有界 IPA 工作树 | Unix 库内已实现 Receipt/`SC_Info` 排除与自动清理；尚无 CLI 或产物发布 |
| FAT/FAT64 对抗解析覆盖 | 已实现 |
| 版本化 Manifest/Capability/Error Schema | 已实现 |
| 首方 DemoLab Simulator Fixture | 已实现 |
| 有界协议规范 | 设计已接受；未实现 |
| 受保护首方 Oracle | LAB-001 得到有界 No-Go；LAB-002 已完成三个可执行文件完整安装 Slice 清单的无设备设计与实现。私有检查点 3 已生成恰好一个通过验证的本地签名 DemoLab `1.0 (3)` 候选/冻结 Oracle Pair，其恰好一次内部 TestFlight 上传也已对账为 Apple 接受并处理。[检查点 4 台账](lab-002-checkpoint-4-progress.md)记录 Host 流程和截至 PR #81 的修复。最后一次全新 4C 仪式中，TestFlight 显示并打开首方 DemoLab `1.0 (3)`，该 App 导入信封并导出签名 Receipt，但 Host 在发布 Enrollment Binding 前停止，所以已安装二进制 Lineage 仍未验证，随后授权过期。检查点 4 已以保留证据的流程性 No-Go 关闭；没有观察 Run、受保护到明文结果或 Go 结果 |
| 设备发现与 Transport | 未实现 |
| 设备 Helper 与 Backend | 未实现 |
| 确定性未签名分析 IPA 打包 | Unix 库内已从保留的私有工作树实现；尚无 CLI 或产物发布 |
| 无设备打包证据 Manifest | Unix 库内已实现 Version-3 Builder，包含 Archive/清单/逐 Code Hash 与完整 Slice；尚无 CLI 发布或明文声明 |
| Mach-O 重建 | 未实现 |
| `oprobe decrypt` | 未实现 |
...
5. 读 [IPA 主程序检查契约（英文）](../development/ipa-main-executable.md)，再跟踪
   ...
   清单漂移和 Mach-O 测试。
6. 读[嵌套 Bundle 元数据契约（英文）](../development/ipa-nested-bundles.md)，再跟踪
   `discover_bundle_roots`、`select_bundle_plists`、
   `inspect_ipa_nested_bundle_metadata` 和 Scope/Limit 测试。
7. 读 [声明 Code 清单契约（英文）](../development/ipa-code-inventory.md)，再跟踪
   `discover_candidates`、`validate_candidate_set`、`inspect_ipa_code_inventory`
   及角色/拒绝测试。
8. 读[私有有界 IPA 工作树契约（英文）](../development/ipa-private-worktree.md)，再跟踪
   `build_worktree_plan`、`open_verified_directory`、
   `materialize_ipa_private_worktree` 和清理/对抗测试。
9. 读[确定性 IPA 打包契约（英文）](../development/ipa-deterministic-package.md)，再跟踪
   `package_records`、`validate_exact_tree`、`package_ipa_analysis_worktree`
   及确定性/对抗测试。
10. 读[无设备打包证据 Manifest 契约（英文）](../development/ipa-package-manifest.md)，
   再跟踪 `build_ipa_package_manifest`、`bind_code_evidence`、
   `inventory_digest` 和 Schema/Golden 测试。
11. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
12. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
13. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v3.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
14. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
15. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
16. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
