...
> Rust CLI 基础、有界 Mach-O 解析、IPA Archive 元数据预检和有界内存 Entry
> 读取、根 App XML/Binary plist Event 的有界身份解析、版本化 Schema、合成
> DemoLab Fixture 和有界协议规范，但还没有设备 Transport、Helper、解密后端、
> 重建器或 IPA Packager。
...
当前库实现了本阶段的只读元数据预检和单个 Entry 的有界内存读取，代码位于
...
Buffer，并检查压缩输入、实际输出、CRC 和声明长度。它不向磁盘解包，也未接入
CLI。精确契约见
...
| `crates/orchardprobe-core/src/ipa.rs` | 只读 ZIP/ZIP64 预检、确定性 IPA 清单和单个有界 CRC 校验内存 Entry 读取。 |
...
未来 Transport、Catalog、Backend、Reconstruction、Archive 物化与 Report 模块
...
| 有界 Stored/Deflate IPA Entry 读取 | 库内存读取已实现；尚未接入 CLI |
...
5. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
6. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
7. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v2.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
8. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
9. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
10. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
