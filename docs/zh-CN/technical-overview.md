...
候选；只有同一 Parser 通过后才称为 Code，误报和畸形候选保持可见。嵌套 Bundle
元数据现已能解析非标准可执行名，但 Catalog 尚未消费这些声明，因此 Coverage
仍明确是不完整的。精确契约见
...
| 有界嵌套 Framework/Extension plist 元数据 | 库内已实现；只定位精确声明 Entry，尚未执行嵌套 Mach-O 晋级或接入 CLI |
| IPA 根主程序 Mach-O 元数据 | 库内已实现；尚未接入 CLI |
| 约定路径 Framework/dylib/Extension Code 候选 | 明确不完整的库内清单已实现；仍需声明 Entry 集成和 CLI 接入 |
...
7. 读 [Code 候选清单契约（英文）](../development/ipa-code-inventory.md)，再跟踪
