...
> 仅供分析 IPA 的库内 Packager；不支持的覆盖范围保持显式。仓库另有版本化
> Schema、合成 DemoLab Fixture 和有界协议规范，但还没有设备 Transport、Helper、
> 解密后端、Mach-O 重建器、产物发布器或 `oprobe decrypt` 命令。
...
调用方指定目标、输入/输出 Hash 与 Manifest 绑定、二进制 Coverage 证据及原子发布
为 `*.decrypted.ipa` 仍属于后续步骤。把未改变的 Fixture 字节打包成功不能证明
Mach-O 已经重建或解密。
...
未来 Transport、Backend、Mach-O Reconstruction、证据/Report 绑定与原子发布模块
...
9. 读[确定性 IPA 打包契约（英文）](../development/ipa-deterministic-package.md)，再跟踪
   ...
   及确定性/对抗测试。
10. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
11. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
12. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v2.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
13. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
14. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
15. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
