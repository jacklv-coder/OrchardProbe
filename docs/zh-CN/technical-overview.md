...
CLI；Plist 解析、可执行文件清单和 Archive 物化仍未实现。精确契约见
...
| 有界 Stored/Deflate IPA Entry 读取 | 库内存读取已实现；尚无 Plist 解析或 CLI 接入 |
...
4. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
5. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
6. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v2.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
7. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
8. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
9. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
