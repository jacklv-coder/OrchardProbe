...
> 有界声明标准 Bundle 清单；不支持的覆盖范围保持显式。仓库另有版本化 Schema、
...
它只返回根元数据；独立的声明标准 Bundle 清单会消费这些结果，Archive 物化仍
未实现。精确契约见
...
7. 读 [声明 Code 清单契约（英文）](../development/ipa-code-inventory.md)，再跟踪
   ...
   及角色/拒绝测试。
8. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
9. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
10. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v2.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
11. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
12. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
13. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
