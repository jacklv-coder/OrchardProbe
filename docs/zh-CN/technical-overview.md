...
> Rust CLI 基础、有界 Mach-O 与 IPA Archive 元数据解析、版本化 Schema、合成
...
不解压或物化 Entry。精确契约见
[IPA 预检开发文档（英文）](../development/ipa-preflight.md)。
...
| `crates/orchardprobe-core/src/ipa.rs` | 只读、有界的 ZIP/ZIP64 元数据预检和确定性 IPA Entry 清单。 |
...
3. 先读 [IPA 预检契约（英文）](../development/ipa-preflight.md)，再跟踪
   `crates/orchardprobe-core/src/ipa.rs` 中的 `read_footer`、
   `read_central_directory`、`validate_local_header` 和对抗测试。
