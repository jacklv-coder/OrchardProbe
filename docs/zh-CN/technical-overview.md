...
> Rust CLI 基础、有界 Mach-O 元数据解析、版本化 Schema、合成 DemoLab
> Fixture 和有界协议规范，但还没有设备 Transport、Helper、解密后端、重建器
> 或 IPA Packager。
...
IPA 是不可信输入。未来 Archive 模块必须先检查 Entry，再物化文件，并为 Entry
...
未来 Transport、Catalog、Backend、Reconstruction、Archive 与 Report 模块只有在
各自设计和证据门槛满足后才能加入。图中的名字表示职责，不表示已有 Crate。
...
3. 从 `main`、`inspect`、`open_regular_file` 阅读
   `crates/orchardprobe-cli/src/main.rs`，了解 CLI 错误和 Host 文件安全惯例。
4. 阅读 `crates/orchardprobe-core/src/macho.rs`：先看 `parse_macho`，再跟踪
   `parse_fat`、`parse_slice`、Range Helper 和对抗测试。
5. 把 `crates/orchardprobe-core/src/lib.rs` 与
   `schemas/v0/export-manifest-v2.schema.json` 对照，理解 Rust 不变量和 Wire Contract。
6. 阅读 `wire.rs`、Schema 指南、Golden 与 Invalid Fixture。
7. 按 `fixtures/DemoLab/README.md` 构建 DemoLab，只检查项目生成的二进制。
8. 先读 RFC-0001，再读 RFC-0002，最后读兼容性政策和测试记录，理解为什么设备
   实现仍被证据门槛阻塞。
